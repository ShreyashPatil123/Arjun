...
describe("prepareModelForSimpleCompletion", () => {
  ...
  // ARJUN sovereign build: the two Codex it.each blocks that stood here are
  // removed with prepareCodexSimpleTransportModel. They asserted that
  // openai-chatgpt-responses models route to ChatGPT's backend-api and that
  // Codex proxy base URLs are validated -- behaviour this build does not have,
  // because that vendor path is not vendored. Coverage that the dispatcher now
  // refuses cloud APIs lives in provider-transport-stream.test.ts.
