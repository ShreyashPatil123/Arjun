...
const registerBuiltIns: RegisterBuiltIn[] = [
  // ARJUN sovereign build: only the OpenAI-compatible transport is registered.
  //
  // Upstream OpenClaw also registers anthropic-messages, mistral-conversations,
  // openai-responses, azure-openai-responses, openai-chatgpt-responses,
  // google-generative-ai and google-vertex. Every one of those reaches a vendor
  // endpoint, so they are removed here rather than merely left unconfigured:
  // their provider entrypoints are not vendored and their SDKs are not
  // dependencies of this package.
  //
  // This single entry is what llama-server (C++/GGUF), vLLM and SGLang (Python)
  // all speak, so one adapter covers both halves of the model estate.
