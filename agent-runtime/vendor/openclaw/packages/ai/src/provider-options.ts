// ARJUN sovereign build: the Anthropic provider is not vendored and
// @anthropic-ai/sdk is not a dependency, so the client handle below is left
// opaque. AnthropicOptions itself is kept because the shared transport layer
// still references its shape.
type Anthropic = never;
