...
impl ModelServers {
    ...
    async fn managed_endpoint(
        ...
        child_cmd
            ...
            .stderr(Stdio::piped())
            .kill_on_drop(true);
        // On Windows, default child processes inherit a console window. For a
        // background model server we explicitly do not want one flashing on
        // the operator's desktop. Same trick `process_utils.rs` uses.
        #[cfg(target_os = "windows")]
        {
